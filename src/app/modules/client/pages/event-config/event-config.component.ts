...
export class EventConfigComponent {
  ...
  constructor(
    ...
    this.titleService.setTitle('Configuración preregistro')
    this.route.queryParamMap.subscribe(params => {
      const eventCode = params.get('eventId');
      if (eventCode) {
  ...
  ngOnInit(): void {
    this.eventService.event.subscribe(data => {
      ...
      if (data) {
        ...
        this.eventService.getInfoVariable(this.eventConfig.event.eventId, 'preRegister').subscribe({
          next: (data) => {
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      const { tipoEvento, tipoDispositivo, tipoVotacion, escarapela, infoCliente, email, phone } = this.form.getRawValue();
